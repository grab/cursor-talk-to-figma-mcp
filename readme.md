...
This creates MCP configurations for all supported integrations:
...
Install from [Figma community page](https://www.figma.com/community/plugin/1485687494525374295/cursor-talk-to-figma-mcp-plugin) or [install locally](#figma-plugin-local-install)
...
Configuration is auto-generated at `.cursor/mcp.json`. No additional setup required.

### GitHub Copilot in VS Code

1. Requires VS Code 1.102+ with Copilot
2. Enable Agent Mode in Copilot settings
3. Configuration at `.vscode/mcp.json` (auto-generated by setup)
4. Use `@workspace` or agent mode to access MCP tools

### Google Antigravity

1. Uses VS Code's native MCP support  
2. Configuration at `.vscode/mcp.json` (auto-generated by setup)
3. MCP tools are automatically available

### Claude Code (VS Code & Antigravity)

1. Auto-discovers configurations from `.vscode/mcp.json`
2. No additional setup required after running `bun setup`
...
**For VS Code/Copilot/Antigravity/Claude:** `.vscode/mcp.json`
...
For local development, point your MCP config to the local server:
...
```json
{
  "mcpServers": {
    "TalkToFigma": {
      "command": "bun",
      "args": ["/path-to-repo/src/mcp_server/server.ts"]
    }
  }
}
```
